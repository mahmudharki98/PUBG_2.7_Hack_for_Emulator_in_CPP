...
bool ESP::loadFont()
{
	if (!font.loadFromFile("./GoodDie.ttf"))
	{
		Sleep(1000);
		if (font.loadFromFile("GoodDie.ttf"))
		{
...
void ESP::setDrawings()
{
	...
	if (playAlert)
	{
		alert.openFromFile("./GoodDieAlert.wav");
		alert.setVolume(80);
		alert.play();
		playAlert = false;
	}
...
void ESP::init()
{
	...
	tmpGName = g_pMemoryManager->dRead(g_pMemoryManager->dRead(gNames) + 0x88);
...
namespace Decryption
{
	...
	uint32_t DecryptActorsArray(uint32_t uLevel, int Actors_Offset, int EncryptedActors_Offset)
	{
		...
		if (g_pMemoryManager->ui32tRead(uLevel + Actors_Offset) > 0)
			return uLevel + Actors_Offset;

		if (g_pMemoryManager->ui32tRead(uLevel + EncryptedActors_Offset) > 0)
			return uLevel + EncryptedActors_Offset;

		auto Encryption = g_pMemoryManager->act_encRead(uLevel + EncryptedActors_Offset + 0x0C);

		if (Encryption.Enc_1 > 0)
		{
			auto Enc = g_pMemoryManager->act_encRead(Encryption.Enc_1 + 0x80);

			return ((g_pMemoryManager->u_i8Read(Encryption.Enc_1 + Enc.Enc_1))
				| ((g_pMemoryManager->u_i8Read(Encryption.Enc_1 + Enc.Enc_2)) << 8)
				| ((g_pMemoryManager->u_i8Read(Encryption.Enc_1 + Enc.Enc_3)) << 16)
				| ((g_pMemoryManager->u_i8Read(Encryption.Enc_1 + Enc.Enc_4)) << 24));
		} //ok
		else if (Encryption.Enc_2 > 0)
		{
			auto Encrypted_Actors = g_pMemoryManager->ui32tRead(Encryption.Enc_2);
		...
		{
			auto Encrypted_Actors = g_pMemoryManager->ui32tRead(Encryption.Enc_3);
			if (Encrypted_Actors > 0)
				return __ROR4__(Encrypted_Actors, 0x18);
		}
		else if (Encryption.Enc_4 > 0)
		{
			auto Encrypted_Actors = g_pMemoryManager->ui32tRead(Encryption.Enc_4);
			if (Encrypted_Actors > 0)
				return Encrypted_Actors ^ 0xCDCD00;
		}
		else
		{
			cout << "Cant Find Actors List" << endl;
		}
		return 0;
	}
}
...
{
	uWorlds = g_pMemoryManager->dRead(g_pMemoryManager->dRead(uWorld) + 0x3C); // 60
	...
	if (playerCarry == 0) { return; }
	uMyObject = g_pMemoryManager->dRead(playerCarry + ofUMyObject);
	// Comment next line for spectating mode
	// if (uMyObject == 0) { return; }
	entityEntry = g_pMemoryManager->dRead(uLevel + ofEntityEntry);
	if (entityEntry == 0) { return; }
	entityCount = g_pMemoryManager->iRead(uLevel + ofEntityCount);

	enemies = 0;

	auto ActorsPointerAddress = Decryption::DecryptActorsArray(uLevel, 0x70, 0x334);//--Actors
	DWORD ActorArray = g_pMemoryManager->dRead(ActorsPointerAddress);
	int ActorArrayCount = g_pMemoryManager->iRead(ActorsPointerAddress + 0x4);
	if (ActorArrayCount > 2048)
		ActorArrayCount = 2048;

	for (int i = 0; i < ActorArrayCount; i++)
	{
		entityAddv = g_pMemoryManager->dRead(ActorArray + i * 0x4);
		if (!entityAddv)
			continue;
		if (entityAddv == (DWORD)nullptr || entityAddv == -1 || entityAddv == NULL)
			continue;

		bool bAutoDestroyWhenFinished = g_pMemoryManager->bRead(entityAddv + ofBAutoDesFinish);
		if (bAutoDestroyWhenFinished == 62) continue; //58 Normal, 62 Hidden

		int entityStruct = g_pMemoryManager->iRead(entityAddv + 0x10);
		if (entityStruct == 0 || entityStruct < 0) continue;

		DWORD fNamePtr = g_pMemoryManager->dRead(tmpGName + int(entityStruct / 0x4000) * 0x4);
		DWORD fName = g_pMemoryManager->dRead(fNamePtr + int(entityStruct % 0x4000) * 0x4);
		string entityType = g_pMemoryManager->charRead(fName + 0x8);
		...
		pEntityName = entityName.c_str();


		// ITEMS
		if (
			strstr(pEntityName, "BP_Grenade") ||
			strstr(pEntityName, "_Wrapper") || strstr(pEntityName, "BP_WEP_Mk14_Pickup_") ||
			strstr(pEntityName, "BP_Ammo") ||
			strstr(pEntityName, "BP_MZJ") ||
			strstr(pEntityName, "PickUp_BP") ||
			strstr(pEntityName, "_Pickup") ||
			strstr(pEntityName, "PlayerDeadInventoryBox_") ||
			strstr(pEntityName, "PickUpListWrapperActor") ||
			strstr(pEntityName, "BP_AirDropBox_") ||
			strstr(pEntityName, "AirDropListWrapperActor") ||
			strstr(pEntityName, "VH_") ||
			strstr(pEntityName, "BP_AirDropPlane_") ||
			strstr(pEntityName, "LadaNiva") ||
			strstr(pEntityName, "AquaRail") ||
			strstr(pEntityName, "Rony") ||
			strstr(pEntityName, "Mirado") ||
			strstr(pEntityName, "PickUp_")
			)
		{
			...
			if (strstr(pEntityName, "BP_Grenade"))
			{
			...
			else if (strstr(pEntityName, "_Wrapper") || strstr(pEntityName, "BP_WEP_Mk14_Pickup_"))
			{
			...
			else if (strstr(pEntityName, "BP_Ammo"))
			{
			...
			else if (strstr(pEntityName, "BP_MZJ"))
			{
			...
			else if (strstr(pEntityName, "PickUp_BP"))
			{
			...
			else if (strstr(pEntityName, "_Pickup"))
			{
				entityName = replace(entityName, "_Pickup_C");
		...
		// PLAYERS
		else if (strstr(pEntityName, "BP_PlayerPawn") || strstr(pEntityName, "BP_PlayerCharacter") || strstr(pEntityName, "PlanET_FakePlayer") || strstr(pEntityName, "PlayerPawn_Infec") || strstr(pEntityName, "Zombie80Weapon") || strstr(pEntityName, "Zombie81Weapon") || strstr(pEntityName, "Zombie82Weapon") || strstr(pEntityName, "Zombie83Weapon") || strstr(pEntityName, "Zombie84Weapon") || strstr(pEntityName, "DBZ_FakePlayerAIPawn"))
		{
			entityWorld = g_pMemoryManager->dRead(entityAddv + ofEntityWorld);
			...
			bonePosition = getBonePos(35); // right hand
			...
			bonePosition = getBonePos(5); // head
			...
			bonePosition = getBonePos(14); // left hand
			armsLegs[6].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			g_pOverlay->window.draw(armsLegs);

			bonePosition = getBonePos(60); // right foot
			armsLegs[0].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(59); // right knee
			armsLegs[1].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(58); // right waste
			armsLegs[2].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(1); // body middle
			armsLegs[3].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(54); // left waste
			armsLegs[4].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(55); // left knee
			armsLegs[5].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			bonePosition = getBonePos(56); // left foot
			armsLegs[6].position = sf::Vector2f(bonePosition.x, bonePosition.y);
			g_pOverlay->window.draw(armsLegs);
