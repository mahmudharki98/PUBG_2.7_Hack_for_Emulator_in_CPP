...
uint32_t _ui32t = 0x0;
uint32_t MemoryManager::ui32tRead(DWORD base)
{
	if (ValidPointer(base))
		readMemory((PVOID)base, &_ui32t, sizeof(uint32_t));
	return _ui32t;
}
unsigned __int8 _u_i8 = 0x0;
unsigned __int8 MemoryManager::u_i8Read(DWORD base)
{
	if (ValidPointer(base))
		readMemory((PVOID)base, &_u_i8, sizeof(unsigned __int8));
	return _u_i8;
}
ActorsEncryption _enc_chn;
ActorsEncryption MemoryManager::act_encRead(DWORD base)
{
	if (ValidPointer(base))
		readMemory((PVOID)base, &_enc_chn, sizeof(ActorsEncryption));
	return _enc_chn;
}
